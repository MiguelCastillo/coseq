...
> subroutines return one value and coroutines can yield multiple values before running to completion.

The contract with `coseq` are iterators, which is why anything that can create an interator can be used with `coseq`. This includes `array`, `Set`, `Map`... etc.
